set MINGW_W64_DIR=%ProgramFiles%\mingw64
set PATH=%MINGW_W64_DIR%\bin;%PATH%

rem set PROCESSOR_ARCHITECTURE=x86
set FBC_VER=_FBC1100
set GCC_VER=_Clang1600
set MARCH=atom
set FBC="C:\Programming\FreeBASIC Projects\fbc\src\compiler\fbc32_icase.exe"
set CC="C:\Program Files\LLVM\bin\clang.exe"
set AS="C:\Program Files (x86)\mingw32\bin\as.exe"
set LD="C:\Program Files (x86)\mingw32\bin\ld.exe"
set AR="C:\Program Files (x86)\mingw32\bin\ar.exe"
set GORC="C:\Program Files (x86)\FreeBASIC-1.10.0-winlibs-gcc-9.3.0\bin\win32\GoRC.exe"
set DLL_TOOL="C:\Program Files (x86)\mingw32\bin\dlltool.exe"

rem Without quotes:
set LIB_DIR=C:\Program Files (x86)\FreeBASIC-1.10.0-winlibs-gcc-9.3.0\lib\win32
set INC_DIR=C:\Program Files (x86)\FreeBASIC-1.10.0-winlibs-gcc-9.3.0\inc
set LD_SCRIPT=C:\Program Files (x86)\FreeBASIC-1.10.0-winlibs-gcc-9.3.0\lib\win32\fbextra.x

set TARGET_TRIPLET=i686-pc-windows-gnu
set USE_RUNTIME=FALSE
